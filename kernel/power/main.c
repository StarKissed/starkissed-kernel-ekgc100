...
#if defined(CONFIG_MACH_GC1) && defined(CONFIG_DVFS_LIMIT)
#define CONFIG_CAMERA_LOWTEMP_LIMIT_SUPPORT
#endif
...
#ifdef CONFIG_CAMERA_LOWTEMP_LIMIT_SUPPORT
static int camera_lowtemp_limit_val;

DEFINE_MUTEX(camera_lowtemp_limit_mutex);

static ssize_t camera_lowtemp_limit_show(struct kobject *kobj,
					struct kobj_attribute *attr,
					char *buf)
{
	return sprintf(buf, "%d\n", camera_lowtemp_limit_val);
}

static ssize_t camera_lowtemp_limit_store(struct kobject *kobj,
					struct kobj_attribute *attr,
					const char *buf, size_t n)
{
	int val;
	...
	mutex_lock(&camera_lowtemp_limit_mutex);

	if (sscanf(buf, "%d", &val) != 1) {
		pr_info("%s: Invalid camera_lowtemp_limit on, off format\n", \
			__func__);
		goto out;
	}

	if (val == 0 && camera_lowtemp_limit_val) {
		exynos_cpufreq_upper_limit_free(DVFS_LOCK_ID_CAMERA_LOWTEMP_LIMIT);
		camera_lowtemp_limit_val = 0;
	}
	else if (val == 1 && !camera_lowtemp_limit_val) {
		exynos_cpufreq_upper_limit(DVFS_LOCK_ID_CAMERA_LOWTEMP_LIMIT, L11);
		camera_lowtemp_limit_val = 1;
	}
	else
		pr_info("%s: camera_lowtemp_limit request is invalid\n", __func__);

	ret = n;
out:
	mutex_unlock(&camera_lowtemp_limit_mutex);
	return ret;
}
power_attr(camera_lowtemp_limit);
#endif /* CONFIG_CAMERA_LOWTEMP_LIMIT_SUPPORT */
...
#endif
#ifdef CONFIG_CAMERA_LOWTEMP_LIMIT_SUPPORT
	&camera_lowtemp_limit_attr.attr,
#endif
	NULL,
};
